  ...
  useEffect(() => {
    if (stats && currentAlgorithm) {
      const algorithmLabel =
        currentAlgorithm === "astar" ? "A*" : currentAlgorithm.toUpperCase();
    ...
    <div className="flex flex-col items-center gap-8">
      ...
      >
        ...
        <p className="text-muted-foreground text-sm max-w-2xl">
          Visualize how different pathfinding algorithms explore a maze to find
          the shortest path
        </p>
        {currentAlgorithm && (
            ...
            <div className="flex items-center gap-2">
              {isRunning ? (
                <>
                  ...
                  <div
                    className={`size-2 rounded-full ${isPaused ? "bg-amber-500" : "bg-green-500 animate-pulse"}`}
                  />
                </>
              ) : (
